...
class Trainer:
    def __init__(self,
        ...
        if use_multi_gpu:
            ...
            with self.mirroed_strategy.scope():
                self._model    = model
    ...
    def training_process(self) -> NoReturn:
        for epoch in range(self._epoch_number):
            total_loss_value = 0
            training_count   = 0
            ...
            for batch, dataset in enumerate(self.data_generator):
                ...
                loss_value = self._training_method(training_data,target_data)
    ...
    def train_step(self, train_data: tf.Tensor, target_data: tf.Tensor):
        with tf.GradientTape() as tape:
            prediction = self.model.pipeline(encoder_input= train_data, decoder_input = target_data)
            loss_value = self.loss_function(prediction, target_data = target_data)
    ...
    def will_save(self, epoch_number: int) -> bool:
        if self._save_times is None:
            return False
        return (epoch_number + 1) % self._save_times == 0
