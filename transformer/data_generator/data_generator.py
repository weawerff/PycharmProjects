...
from ospark.data.encoder import LanguageDataEncoder, SubwordTextEncoder
import math
...
class TranslateDataGenerator(DataGenerator):
    ...
    def create_from_dataset(cls,
                            train_data:list[str],
                            target_data:list[str],
                            target_data_vocab_size:int,
                            train_data_vocab_size:int,
                            batch_size: int,
                            max_token:Optional[list] = None,
                            max_length: Optional[list] = None,
                            start_index: Optional[list] = None,
                            padding_range: Optional[list] = None) -> TranslateDataGenerator:
        train_data_encoder = SubwordTextEncoder.build_from_corpus(train_data, target_vocab_size = train_data_vocab_size)
        label_data_encoder = SubwordTextEncoder.build_from_corpus(target_data, target_vocab_size = target_data_vocab_size)
        data_encoder = LanguageDataEncoder(train_data_encoder = train_data_encoder, label_data_encoder = label_data_encoder)
        return cls(training_data = train_data,
                   target_data = target_data,
                   data_encoder = data_encoder,
                   max_length = max_length,
                   max_token = max_token,
